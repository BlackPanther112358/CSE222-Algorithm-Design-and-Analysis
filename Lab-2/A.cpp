...
void solve(){
    ...
    cin >> n;
    vii dp(n, {0, 0}), a(n);
    rep_u(i, 0, n) cin >> a[i].first >> a[i].second;
    dp[0] = {1, 1};
    rep_u(i, 1, n){
        if(a[i].first != a[i - 1].first) dp[i].first += dp[i - 1].first;
        if(a[i].first != a[i - 1].second) dp[i].first += dp[i - 1].second;
        dp[i].first %= mod;
        if(a[i].second != a[i - 1].first) dp[i].second += dp[i - 1].first;
        if(a[i].second != a[i - 1].second) dp[i].second += dp[i - 1].second;
        dp[i].second %= mod;
    }
    cout << (dp[n - 1].first + dp[n - 1].second)%mod << nline;
    return;
}
